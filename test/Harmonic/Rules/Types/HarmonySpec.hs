...
module Harmonic.Rules.Types.HarmonySpec (spec) where

import Test.Hspec

import Harmonic.Rules.Types.Pitch (PitchClass(..), NoteName(..), flat, sharp, unPitchClass)
...
spec = do
  ...
  describe "walkTriadCadence / walkTriadState (corpus-shaped walk projection)" $ do
