...
spec = do
  changeSpec
  describe "edgeRow (write/read contract, no database)" $ do
...
changeSpec :: Spec
changeSpec =
  describe "changeBatchCypher" $ do
    it "is a batched UNWIND statement with no value splicing" $ do
      changeBatchCypher `shouldSatisfy` T.isInfixOf "UNWIND $rows"
      changeBatchCypher `shouldSatisfy` (not . T.isInfixOf "'")
    it "is label-scoped to Change on both endpoints" $ do
      changeBatchCypher `shouldSatisfy` T.isInfixOf "(from:Change"
      changeBatchCypher `shouldSatisfy` T.isInfixOf "(to:Change"
      changeBatchCypher `shouldSatisfy` (not . T.isInfixOf ":Cadence")
