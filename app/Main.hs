...
type Harmony = ([Chord], [Cadence]) -- 'vertical' deterministic slice of context sensitive harmony options
...
--  return Progression (enharm, cadences)
...
chainCadence enharm state context len entropy = do
  ...
  chords            <- fst cadenceSequence
  cadences          <- snd cadenceSequence
  let
    chords enharm   = showTriad enharm <$> fst chords
    chordLen        = length $ chords enharm :: Int
    chordLines      = (++"|   ") . concat . (`replicate`" ") .
                      ((14-) . length) <$> chords enharm
    fours enharm    = concat $ zipWith (++)
  ...
  liftIO $ putStrLn "generated by 'The Harmonic Algorithm' -> https://github.com/OscarSouth/theHarmonicAlgorithm"
  liftIO $ putStr (fours enharm) >> putStrLn "|\n"
  return Progression (enharm, chords, cadences)

-- |just in case I need it
--      chordLists    = show . fromChord <$> fst cadences
--      listLines     = (++"|   ") . concat . (`replicate`" ") .
--                      ((14-) . length) <$> chordLists
--      lists         = concat $ zipWith (++)
--                      ["\n    ||   ", "\n   5    |   ", "\n   9    |   ", "\n   13   |   "]
--                      (init . init . init <$> (fmap concat <$> chunksOf 4 $
