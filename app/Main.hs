...
--type Harmony ([Chord], [Cadence])= ([Chord], [Cadence]) -- 'vertical' deterministic slice of context sensitive harmony options
...
--  chords <- orderVoicing . fromChord <$> fst progression
--  return chords
...
harmony :: ([Chord], [Cadence]) -> IO [[Int]]
harmony progression = do
  chords <- map (orderVoicing . fromChord) <$> pure (fst progression)
  return chords
