...
type Filters = ((PitchClass -> NoteName) -> [Chord])
...
harmonicFilters :: ReaderT MarkovMap IO Filters
harmonicFilters = do
...
markovLoop :: Enharmonic -> Root -> Cadence -> Filters -> Int ->
    ...
    then do
      ...
      markovLoop fs root prev filters n
        ...
        markovLoop fs root prev filters' n
          ...
          then do
          liftIO $ putStrLn ""
          markovLoop fs root prev filters $ if n == 15 then 35 else 15
