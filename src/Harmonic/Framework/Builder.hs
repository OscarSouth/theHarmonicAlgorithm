...
import           Harmonic.Traversal.Probabilistic (gammaIndexScaled)
import           Harmonic.Rules.Constraints.Filter (parseOvertones, parseOvertones', parseKey, parseFunds, isWildcard, resolveRoots)
import           Harmonic.Rules.Constraints.Overtone (overtoneSets, nCr)
...
-------------------------------------------------------------------------------
-- Portmanteau Generation for Composer Display
-------------------------------------------------------------------------------
...
-- Now takes raw Text instead of parsed ComposerWeights
printHeader :: Text -> HarmonicContext -> IO ()
printHeader composerStr ctx = do
  case makePortmanteau composerStr of
    Just portmanteau ->
      putStrLn $ "t> |= \"" ++ T.unpack portmanteau
              ++ "\" =| traversal, generated by -> https://github.com/OscarSouth/theHarmonicAlgorithm"
    Nothing ->
...
gen start len composerStr entropy ctx = do
  ...
  putStrLn ""
  printHeader (T.pack composerStr) ctx
  print prog
  putStrLn ""
  pure prog
