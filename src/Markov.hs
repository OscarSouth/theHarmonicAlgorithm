...
cadenceCounts xs = 
  let mapInsert acc key = Map.insertWith (+) key 1 acc
   in foldl mapInsert Map.empty $ trigrams xs
...
-- nextState st = fromMaybe [] $ Map.lookup st $ markovMap tsData

-- state = (Cadence Maj, Cadence Min)

-- test = zip ["a", "c", "b"] [1 ,3 , 2]

-- test' ts = List.sortBy (comparing snd) ts
